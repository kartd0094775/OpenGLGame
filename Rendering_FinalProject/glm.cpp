...
glmReadMTL(GLMmodel* model, char* name)
{
	...
	strcpy_s(filename, strlen(dir) * 4, dir);
	strcat_s(filename, strlen(name) * 4, name);
	free(dir);
	...
	while (fscanf_s(file, "%s", buf, sizeof(buf)) != EOF) {
		...
		case 'n':				/* newmtl */
			fgets(buf, sizeof(buf), file);
			nummaterials++;
			sscanf_s(buf, "%s", buf, _countof(buf));
			break;
		default:
			/* eat up rest of line */
			fgets(buf, sizeof(buf), file);
...
glmWriteMTL(GLMmodel* model, char* modelpath, char* mtllibname)
{
	...
	strcpy_s(filename, strlen(filename), dir);
	strcat_s(filename, strlen(filename), mtllibname);
	free(dir);
